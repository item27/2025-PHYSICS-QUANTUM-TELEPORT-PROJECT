...
import { advanceSession, createSession, fetchSession, joinSession, leaveSession } from '@services/api';
import { connectToSession } from '@services/websocket';
import { ClientStatus, LocalView, SessionState, WSMessage } from '@state/network/types';
...
function NetworkPage() {
  ...
  const isComplete = session ? session.stepIndex >= session.steps.length - 1 : false;
  ...
  const handleCreate = async () => {
    if (clientStatus === 'connected' || clientStatus === 'joining' || clientStatus === 'session_loaded') return;
    resetClient();
    try {
  ...
  const handleLookup = async () => {
    if (!sessionIdInput || clientStatus === 'connected' || clientStatus === 'joining') return;
    resetConnection();
    try {
  ...
  const bindWebSocket = (sessionId: string, roleName: string, sessionToken: string) => {
    const ws = connectToSession(sessionId, sessionToken, (payload: WSMessage) => {
  ...
  const handleLeave = async () => {
    ...
    try {
      ...
      setSession(updated);
      setClientStatus('session_loaded');
      setStatus('Роль освобождена. Выберите новую роль или подключите другого участника.');
    } catch (err) {
      console.error(err);
  ...
  const hintByRole: Record<QubitView['role'], string> = {
    ...
    charlie: 'Поддерживает общее запутанное поле',
  };

  const roleToStepGate: Record<string, string[]> = {
    alice: ['combine', 'measure'],
    bob: ['send_classical', 'reconstruct'],
    charlie: ['entangle'],
  };
  ...
  const canAdvance = Boolean(session && token && role && roleToStepGate[role]?.includes(activeStepKey ?? ''));

  const phaseActive = (role: string) =>
    (role === 'alice' && (activeStepKey === 'entangle' || activeStepKey === 'combine')) ||
    (role === 'bob' && activeStepKey === 'reconstruct');

  const decoherence = (role: string) => role === 'alice' && activeStepKey === 'measure';

  const depth = (role: string) => {
    if (role === 'alice' && activeStepKey === 'measure') return 'idle';
    if (role === 'bob' && (activeStepKey === 'reconstruct' || activeStepKey === 'complete')) return 'active';
    return undefined;
  };
  ...
  const stepNotes = {
    entangle: {
      ...
      lines: ['Чарли держит запутанную пару для всех.', 'Это основа протокола: общее состояние двух точек.'],
      icon: 'entangle' as const,
      tone: 'quantum' as const,
    },
    combine: {
      title: 'Вплетение состояния Алисы',
      lines: ['Алиса помещает свой кубит в общее поле.', 'Фаза ещё движется, классики нет.'],
      icon: 'state' as const,
      tone: 'quantum' as const,
    },
    measure: {
      title: 'Коллапс и потеря исходного состояния',
      lines: ['Измерение уничтожает копию у Алисы.', 'Дальше двигаются только классические биты.'],
      icon: 'measure' as const,
      tone: 'warning' as const,
    },
    send: {
      title: 'Классический канал',
      lines: ['Результат измерения — обычные биты.', 'Передача не несёт квантовой информации.'],
      icon: 'classical' as const,
      tone: 'classical' as const,
    },
    reconstruct: {
      title: 'Локальная коррекция у Боба',
      lines: ['Боб применяет операцию только у себя.', 'Исходное состояние рождается на его стороне.'],
      icon: 'correction' as const,
      tone: 'quantum' as const,
    },
    complete: {
      title: 'Завершение протокола',
      lines: ['Исходное состояние закреплено у Боба.', 'У Алисы оно недоступно — процесс необратим.'],
      icon: 'state' as const,
      tone: 'classical' as const,
    },
  };

  const info = activeStepKey ? stepNotes[activeStepKey as keyof typeof stepNotes] : null;
    ...
    <div className={`grid-2 scene scene--${activeStepKey ?? 'idle'}`}>
      <section className="panel">
        <div className="panel-header">
          <div>
            ...
            <p className="subtitle">
              Подключитесь к бэкенду через WebSocket. Сервер — источник истины, клиент лишь отображает состояние.
            </p>
          </div>
          <div className="control-bar">
            ...
            {clientStatus === 'joining' && <button disabled>Подключаемся…</button>}
        ...
        <div className="status-line">
          ...
          {role && clientStatus === 'connected' && <span className="pill">Роль: {role}</span>}
        </div>
        <div className="control-bar control-bar--secondary">
          <button onClick={handleAdvance} disabled={clientStatus !== 'connected' || isComplete || !canAdvance}>
        ...
        {session ? (
          <div className="pill-row">
            {Object.values(session.participants).map((p) => (
              ...
              >
                {p.role} · {p.taken ? 'занята' : 'свободна'} {p.connected ? '· online' : ''}
              </span>
            ))}
          </div>
        ) : (
          <p className="placeholder">Создайте или найдите сессию, чтобы увидеть роли.</p>
        )}
        {session ? (
          <TeleportationSteps steps={session.steps} activeIndex={session.stepIndex} />
        ) : (
          <p className="placeholder">Создайте сессию, чтобы наблюдать обновления в реальном времени.</p>
        )}
      </section>

      <section className="panel">
        <h3>Состояния кубитов</h3>
        <p className="subtitle">Все изменения приходят от сервера. Клиент только показывает картину.</p>
        {session ? (
          <>
            <div className="qubit-grid">
              {session.qubits.map((q) => (
                <QubitCard
                  key={q.id}
                  qubit={q}
                  emphasis={session.steps[session.stepIndex]?.key === 'reconstruct'}
                  phaseActive={phaseActive(q.role)}
                  decoherence={decoherence(q.role)}
                  hint={hintByRole[q.role]}
                  depth={depth(q.role)}
                  showBloch={(q.role === 'alice' && activeStepKey !== 'complete') ||
                    (q.role === 'bob' && (activeStepKey === 'reconstruct' || activeStepKey === 'complete'))}
                />
              ))}
            </div>
            <CircuitView
          ...
          <div className="placeholder">Нет активной сессии. Подключитесь, чтобы увидеть живые данные.</div>
        )}
      </section>
    </div>
  );
