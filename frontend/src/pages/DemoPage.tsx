...
function DemoPage() {
  ...
  const hintByRole: Record<QubitView["role"], string> = {
    alice:
      currentStep.key === "measure"
        ? "Состояние не определено до измерения"
        : "Квантовое состояние готовится к обмену",
    bob:
      currentStep.key === "reconstruct"
        ? "Коррекция - локальная операция"
        : "Ожидает классические биты",
  };

  const phaseActive = (role: string) =>
    (role === "alice" &&
      (currentStep.key === "entangle" || currentStep.key === "combine")) ||
    (role === "bob" &&
      (currentStep.key === "entangle" || currentStep.key === "reconstruct"));

  const decoherence = (role: string) =>
    role === "alice" && currentStep.key === "measure";

  const depth = (role: string) => {
    if (role === "alice" && currentStep.key === "measure") return "idle";
    if (
      role === "bob" &&
      (currentStep.key === "reconstruct" || currentStep.key === "complete")
    )
      return "active";
    return undefined;
  };
  ...
  const stepNotes = {
    entangle: {
      ...
      lines: [
        "Алиса или Боб готовят пару, общую для участников.",
        "Без общего поля протокол невозможен.",
      ],
    ...
    combine: {
      ...
      lines: [
        "Состояние Алисы вплетается в пару.",
        "Фаза ещё жива: никакой классики.",
      ],
      icon: "state" as const,
      tone: "quantum" as const,
    },
    measure: {
      ...
      lines: [
        "Коллапс уничтожает исходное состояние у Алисы.",
        "В кадре остаются только классические биты.",
      ],
    ...
    send: {
      ...
      lines: [
        "Отправляются обычные биты.",
        "Квантовой информации здесь больше нет.",
      ],
    ...
    reconstruct: {
      ...
      lines: [
        ...
        "Исходное состояние проявляется в его кубите.",
      ],
  ...
  return (
    <div className={`grid-2 scene scene--${currentStep.key}`}>
        ...
        <div className="panel-header">
          <div>
            ...
            <p className="subtitle">
              Переключайтесь по шагам и смотрите, как меняется состояние
              кубитов.
            </p>
          </div>
      ...
      <section className="panel">
        ...
        <p className="subtitle">
          Каждый блок - роль участника. Текст описывает их локальное состояние.
        </p>
        <div className="qubit-grid">
          {qubits.map((q) => (
            <QubitCard
              ...
              depth={depth(q.role)}
              showBloch={
                (q.role === "alice" && currentStep.key !== "complete") ||
                (q.role === "bob" &&
                  (currentStep.key === "reconstruct" ||
